 ...
 */
const AppVersion32_t appFirmwareVersion = {.u{.x{
...
jobMessageType_t getJobMessageType(const char *pTopicName, uint16_t topicNameLength) {
  ...
  for (; index < jobMessageTypeMax; index++) {
    mqttStatus = MQTT_MatchTopic(pTopicName, topicNameLength, pJobTopicFilters[index], strlen(pJobTopicFilters[index]),
                                 &isMatch);
...
static void mqttJobCallback(MQTTContext *, MQTTPublishInfo_t *pPublishInfo) {
  ...
  switch (jobMessageType) {
    ...
    default:
      ESP_LOGD(TAG, "Received job message on topic '%.*s', msgSize=%zu.", pPublishInfo->topicNameLength,
               pPublishInfo->pTopicName, pPublishInfo->payloadLength);
...
static void mqttDataCallback(MQTTContext *, MQTTPublishInfo_t *pPublishInfo) {
  OtaEventData_t *pData;
  OtaEventMsg_t eventMsg = {};
...
static OtaMqttStatus_t mqttSubscribe(const char *pTopicFilter, uint16_t topicFilterLength, uint8_t qos) {
  ...
  MQTTSubscribeInfo_t subscribeInfo[NUM_SUBSCRIPTIONS] = {
      {.qos = MQTTQoS::MQTTQoS1, .pTopicFilter = pTopicFilter, .topicFilterLength = (uint16_t)topicFilterLength}};
  ...
  for (int index = 0; index < 2; index++) {
    bool isMatch;
    auto mqttStatus = MQTT_MatchTopic(pTopicFilter, topicFilterLength, pWildCardTopicFilters[index],
                                      strlen(pWildCardTopicFilters[index]), &isMatch);
    assert(mqttStatus == MQTTSuccess);
    if (isMatch) {
      if (index == 0) {
        // Jobs
...
static OtaMqttStatus_t mqttPublish(const char *const pacTopic, uint16_t topicLen, const char *pMsg, uint32_t msgSize,
                                   uint8_t qos) {
  OtaMqttStatus_t otaRet = OtaMqttSuccess;
...
static OtaMqttStatus_t mqttUnsubscribe(const char *pTopicFilter, uint16_t topicFilterLength, uint8_t qos) {
  static const int NUM_SUBSCRIPTIONS = 1;
  MQTTSubscribeInfo_t subscribeInfo[NUM_SUBSCRIPTIONS] = {
      {.qos = (MQTTQoS_t)qos, .pTopicFilter = pTopicFilter, .topicFilterLength = (uint16_t)topicFilterLength}};
...
static bool _validate_new_image() {
  ...
  if (strcmp(CMAKE_THING_TYPE, identity_get()->thing_type) != 0) {
    ESP_LOGE(TAG, "Thing type is incompatible %s vs %s", CMAKE_THING_TYPE, identity_get()->thing_type);
    is_ok = false;
  }

  if (CMAKE_HARDWARE_REVISION_MAJOR != identity_get()->hardware_major) {
    ESP_LOGE(TAG, "Hardware major version is incompatible %d vs %d", CMAKE_HARDWARE_REVISION_MAJOR,
             identity_get()->hardware_major);
    is_ok = false;
  }
...
esp_err_t mqtt_ota_init(EventGroupHandle_t networkEventGroup) {
  ...
  ESP_LOGI(TAG, "Initialising OTA for Firmware version is %" PRIu8 ".%" PRIu8 "-%" PRIu16, appFirmwareVersion.u.x.major,
  ...
  ESP_GOTO_ON_ERROR(esp_event_handler_register(CORE_MQTT_EVENT, ESP_EVENT_ANY_ID, &_event_handler, nullptr), error, TAG,
                    "Failed to register event handler for MQTT");
  ...
  if ((otaRet = OTA_Init(&otaBuffer, &otaInterfaces, reinterpret_cast<const uint8_t *>(identity_thing_id()),
