#include "network_transport_no_tread.h"
...
TlsTransportStatus_t xTlsConnectNoThread(NetworkContext_t *pxNetworkContext) {
  ...
  if (esp_tls_conn_new_sync(pxNetworkContext->pcHostname, strlen(pxNetworkContext->pcHostname), pxNetworkContext->xPort,
...
int32_t espTlsTransportSendNoThread(NetworkContext_t *pxNetworkContext, const void *pvData, size_t uxDataLen) {
...
int32_t espTlsTransportRecvNoThread(NetworkContext_t *pxNetworkContext, void *pvData, size_t uxDataLen) {
  if (pvData == NULL || uxDataLen == 0) {
    return -1;
  }
